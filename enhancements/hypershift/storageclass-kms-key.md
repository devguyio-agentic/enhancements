...
- Propagate the field at cluster creation from `HostedCluster` ->
  ...
  on initial resource creation, not continuously reconciled).
...
- **HCCO:** The Hosted Cluster Config Operator, a subcommand of the CPO binary (from
  the guest release payload), running in the HostedControlPlane namespace on the management cluster.
...
2. The HostedCluster controller creates the `HostedControlPlane` with `operatorConfiguration`
   mirrored.
3. The HCCO creates `ClusterCSIDriver` with `spec.driverConfig.aws.kmsKeyARN` set.
